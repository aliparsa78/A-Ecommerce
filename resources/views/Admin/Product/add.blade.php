    ...
    <x-auth-card>
        ...
        <form method="POST" action="Add_product" enctype="multipart/form-data">
            ...
            <div>
                ...
                <x-input id="input-form" class="block mt-1 w-full" type="float" name="original_price" :value="old('name')" placeholder="Enter the original price of product"  required autofocus  />
            </div>
            <div>
                <x-label id="x-label" for="name" :value="__('selling_price')" class="mt-4"/>
                <x-input id="input-form" class="block mt-1 w-full form-control" type="float" name="selling_price" :value="old('name')" placeholder="Enter the selling price of product"  required autofocus  />
            </div>
            <div>
                <x-label id="x-label" for="name" :value="__('weight')" class="mt-4"/>
                <x-input id="input-form" class="block mt-1 w-full" type="float" name="weight" :value="old('name')" placeholder="Enter the weight of your product" required autofocus  />
            </div>
            
            <div>
                <x-label id="x-label" for="name" :value="__('tax')" class="mt-4"/>
                <x-input id="input-form" class="block mt-1 w-full" type="float" name="tax" :value="old('name')" placeholder="Enter the tax that enact on your product" required autofocus  />
            </div>
            <div>
